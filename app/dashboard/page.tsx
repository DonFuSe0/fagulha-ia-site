...
export default async function DashboardPage() {
  ...
  const { data: profile } = await supabase.from('profiles').select('id, avatar_url, credits, nickname, email').eq('id', user.id).single<Profile>()
  const { data: gens } = await supabase.from('generations').select('id, image_url, thumb_url, is_public, created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(8).returns<Gen[]>()
  const { data: moves } = await supabase.from('tokens').select('id, amount, description, created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(10).returns<Move[]>()
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      <ProfileCard userId={user.id} email={user.email ?? ''} nickname={profile?.nickname ?? null} avatarUrl={profile?.avatar_url ?? null} credits={profile?.credits ?? 0} />
      <section>
